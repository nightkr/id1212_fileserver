...
class UserManager {
  ...
  def create(username: String, password: String)(implicit ec: ExecutionContext): DBIO[Unit] =
  ...
  private def comparePassword(hash: String, password: String): Boolean = hash == password
